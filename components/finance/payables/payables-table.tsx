...
import { Payable } from '@/lib/types/finance';
  ...
  if (data.length === 0) {
    return (
      <div className="border border-uniq-border rounded-xl p-8 text-center">
        <p className="text-uniq-muted">Nenhuma conta a pagar encontrada.</p>
      </div>
    );
  }
            ...
            <TableRow key={item.id} className="group">
              ...
              <TableCell>
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  {item.status === 'paid' && onViewReceipt && (
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-8 w-8 text-uniq-primary"
